...
class RecreateWithAttachments(commands.Cog):
    ...
    async def recreate(self, ctx, source_channel: discord.TextChannel, dest_channel: discord.TextChannel):
        await ctx.send(f"🔄 Recreating messages from {source_channel.mention} to {dest_channel.mention}...")

        # Cache of all threads in the source channel
        source_threads = {t.id: t for t in source_channel.threads}
        ...
        async for msg in source_channel.history(limit=None, oldest_first=True):
            ...
            # ✅ If a thread was created from this message, its ID will match
            thread = source_threads.get(msg.id)
            if thread:
                try:
