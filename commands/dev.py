...
class Dev(commands.Cog):
    ...
    async def rank(self, ctx, user: discord.Member, user_xp: int, max_xp: int):
        ...
        font = ImageFont.truetype('circular-black.ttf', size=46)
        xp_text = f"XP: {user_xp}"
        balance_text = f"Balance: {user_balance}"
        # Add other text as needed

        draw = ImageDraw.Draw(base)
        draw.text((300, 75), xp_text, fill=text_fill, font=font)  # Adjust the position as needed
        draw.text((300, 150), balance_text, fill=text_fill, font=font)  # Adjust the position as needed
        # Add other text as needed
