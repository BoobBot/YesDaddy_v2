...
class ConfirmView(discord.ui.View):
    ...
    async def interaction_check(self, interaction):
        if interaction.user.id != self.uid:
            await interaction.response.send_message(content='You are not allowed to interact with this button.',
                                                    ephemeral=True)
            return False
        return True
...
class LabeledSelect(discord.ui.Select):
    def __init__(self, *, options: list, enabled: list = None, return_list: bool = False, **kwargs):
        ...
        self.all_options = [discord.SelectOption(label=x, value=self.choices[idx], default=enabled[idx]) for idx, x in
                            enumerate(options)]
        self.page = 0
...
class NextPageButton(discord.ui.Button):
    ...
    async def callback(self, interaction):
        ...
        self.labeled_select.options = self.labeled_select.all_options[
                                      self.labeled_select.page * 25:(self.labeled_select.page + 1) * 25]
...
class JailView(discord.ui.View):
    def __init__(self, game, config, choices: list):
        ...
        if 'b' in choices:
            self.add_item(
                LabeledButton('b', label=f'Post Bail (${config["bailValue"]})', style=discord.ButtonStyle.gray))
        if 'g' in choices:
            self.add_item(
                LabeledButton('g', label=f'Use a "Get Out of Jail Free" card ({self.game.goojf[self.game.p]} left)',
...
class TradeView(discord.ui.View):
    def __init__(self, game, config, choices: list, enabled: list):
        ...
        if len(choices) > 0:
            ...
            labeled_select = LabeledSelect(options=choices, enabled=enabled, return_list=True,
                                           max_values=min(25, len(choices)))
            self.add_item(labeled_select)
        self.add_item(LabeledButton('d', label='Done', style=discord.ButtonStyle.blurple))
